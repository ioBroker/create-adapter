...
export async function getTranslatedSettingsForLanguage(
	...
	translatedSettings[`${answers.adapterName} adapter settings`] = titles[language].replace(
		/<adapterName>/gi,
		answers.adapterName,
	);
 ...
 */
export async function getTranslatedSettings(answers: Answers): Promise<Record<string, TranslatedTerm>> {
	const languages: Languages[] = ["en", "de", "ru", "pt", "nl", "fr", "it", "es", "pl", "uk", "zh-cn"];
...
export function getI18nJsonTemplate(language: Languages, forReact: boolean): TemplateFunction {
	return async answers => {
		...
		return formatJsonString(JSON.stringify(translatedSettings, null, 4), answers.indentation || "Tab");
	};
}
