 ...
 */
export async function fetchPackageReferenceVersion(packageName: string): Promise<string> {
	if (hasVersionSpecifier(packageName)) {
		const versionSpecifier = getVersionSpecifier(packageName)!;
		const pureVersionSpec = versionSpecifier.replace(/^[\^~]/, "").trim();
		const versionDepth = pureVersionSpec.replace(/^(0\.)+/, "").split(".").length;
		const version = await fetchSpecificPackageVersion(packageName, pureVersionSpec);
