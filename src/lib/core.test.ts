...
const testAllFiles = async (assertion: (content: string, filename: string) => void) => {
...
describe("./core directory", () => {
	...
	it('should not contain any imports from "os"', async () => {
		await testAllFiles((content, file) =>
			content.should.not.match(/import .+ from "os";/gi, `${file} contains an import from "os"`),
		);
	});

	it('should not contain any imports from "fs"', async () => {
		await testAllFiles((content, file) =>
			content.should.not.match(/import .+ from "fs";/gi, `${file} contains an import from "fs"`),
		);
	});

	it('should not contain any imports from "fs-extra"', async () => {
		await testAllFiles((content, file) =>
			content.should.not.match(/import .+ from "fs-extra";/gi, `${file} contains an import from "fs-extra"`),
		);
	});

	it('should not contain any imports from "path"', async () => {
		await testAllFiles((content, file) =>
			content.should.not.match(/import .+ from "path";/gi, `${file} contains an import from "path"`),
		);
	});
});
